...
const ManejoRutas = () => {
  ...
  const detenerRuta = () => {
    if (watchId) {
      ...
      setEndTime(new Date()); // Registrar la hora de finalización
    }
  };
  ...
  const finalizarRuta = async () => {
    ...
    try {
      ...
      const response = await axios.post('/api/admin/rutas/', {
        ...
        tiempo_estimado: calcularTiempoTranscurrido(), // Usar el tiempo transcurrido calculado
                ...
                <li key={rutas.nombre} className="lista-rutas">
                  ...
                  <div className="botones">
                    <button onClick={() => cargarRutaEnMapa(rutas.nombre)} className="boton-detalle">
                      <img src='/img/info.png' alt="localizacion"/>Detalles Sobre la ruta
                    </button>
                    <button onClick={() => eliminarRuta(rutas.nombre)} className="boton-eliminar">
                      <img src="/img/eliminar.png" alt="eliminar ruta"/>Eliminar ruta
                    </button>              
                  </div>
                </li>
              ))}
